...
import { Activity, Cross, Salad } from 'lucide-react'

const componentClass =
  'p-12 m-4! bg-[var(--color-background-soft)] opacity-80 cursor-pointer transition-all flex flex-col justify-center items-center text-2xl rounded-xl border-1 border-[var(--color-background-dark)] w-82 h-64 hover:opacity-100 hover:shadow-lg gap-4'
  ...
  return (
    <div className="main-screen overflow-y-scroll min-2xl:mx-60! justify-center flex h-full flex-wrap">
      ...
      <div className={` ${componentClass}`}>
        <Cross size={40} color="#7bc1b7" />
        <span className="text-[#07572e]">Diabetes Detection</span>
      </div>
      {activeContent !== 'none' ? (
